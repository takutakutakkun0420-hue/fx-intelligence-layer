...
async def fetch_fx_rates(client: httpx.AsyncClient, start_date: date, end_date: date, from_curr: str, to_curr: str):
    """
    Fetches rates from Frankfurter API using a shared client.
    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 1: 
                logger.error("Network failed after retry. Attempting fallback.")
                return load_fallback_data(start_date, end_date, to_curr)
            logger.warning("Connection failed, retrying...")
        except Exception as e:
            logger.error(f"Unexpected error: {e}. Attempting fallback.")
            return load_fallback_data(start_date, end_date, to_curr)
    return {}
...
async def lifespan(app: FastAPI):
    ...
    # Initialize shared HTTP client for performance (Titanium standard)
...
async def get_summary(
    ...
    # 2. Fetch Data (using shared client)
    target_currency = "USD"
    raw_rates = await fetch_fx_rates(request.app.state.http_client, start_date, end_date, "EUR", target_currency)
    ...
    for d in sorted_dates:
        # Handle { "USD": 1.23 } structure dynamically
        val = raw_rates[d]
        rate_val = val.get(target_currency) if isinstance(val, dict) else val
        
        # Skip if target currency is missing
        if rate_val is None:
            continue
    ...
    if not rates_list:
        raise HTTPException(status_code=404, detail="Valid rates not found")
